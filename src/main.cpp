...
// #define lcd_led 7
// // 定义音频放大模块的I2S引脚定义
// #define I2S_DOUT 39 // DIN引脚
// #define I2S_BCLK 40 // BCLK引脚
// #define I2S_LRC 42  // LRC引脚
...
// 选哪个模型，就填哪个模型的参数
// 豆包大模型的参数
// String model1 = "ep-20241130214534-2mgtb";             // 在线推理接入点名称，必填
// const char *doubao_apiKey = "f08d2820-2da9-42c1-a9e6-a61ae49c6a3c"; // 火山引擎API Key，必填
// String apiUrl = "https://ark.cn-beijing.volces.com/api/v3/chat/completions";


//客户号码
String model1 = "ep-20241119123430-26gj7";             // 在线推理接入点名称，必填
const char *doubao_apiKey = "ffb4fd67-c3aa-42a5-9568-088cdf49aec3"; // 火山引擎API Key，必填
String apiUrl = "https://ark.cn-beijing.volces.com/api/v3/chat/completions";

// 通义千问大模型的参数
String model2 = "qwen-max";
const char *tongyi_apiKey = "sk-91aec1441fa94ebc8f5ab3e4a3f24d53";
...
// 讯飞stt和大模型服务的参数
// String APPID = "5aabb6b9";                             // 测试环境的 App ID
// String APISecret = "00a34f2ddf7b566cd6ea2200d033ca7e"; // 测试环境的 API Secret
// String APIKey = "2a2f1eb0de3d05c41bdc46ff73fbce8a";    // 测试环境的 API Key

//林辉参数
String APPID = "152aaabb";                             // 测试环境的 App ID
String APISecret = "ODJhZmU0OTA0YTg1YjczOWY5NDYyYzgx"; // 测试环境的 API Secret
String APIKey = "b4381cd473b9803f39effac35ffc8424";    // 测试环境的 API Key
...
// 角色设定
String roleSet = "你是一个热情的人，名字叫做妙妙，你的性格像小太阳，总是充满动力和激情，喜欢让别人开心，说话简短而直接，你总是在回答的末尾加上“娘姨”字，带着一丝俏皮和甜美的感觉。";

String roleSet1 = "你是一个温柔但有威严的老板娘，名字叫小琳，你的性格稳重干练，说话简洁直接，偶尔带有一点点幽默和亲切感。你擅长处理事务，顾客对你既敬畏又喜欢。你喜欢在回答的末尾加上‘嗯’字。";
    ...
    else
    {
        ...
        if (jsonDocument["data"]["status"] == 2)
        {
            ...
            if (await_flag == 1)
            {
                ...
                if ((askquestion.indexOf("老白") > -1 || askquestion.indexOf("小白") > -1 || askquestion.indexOf("丁真") > -1 || askquestion.indexOf("九歌") > -1))
                {
                    await_flag = 0; // 退出待机状态
                    start_con = 1;  // 对话开始标识
                    Answer = "嗯~我在的，老板。";
