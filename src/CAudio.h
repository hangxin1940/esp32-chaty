...
extern __attribute__((weak)) void audio_process_extern(int16_t* buff, uint16_t len, bool* continueI2S);
// record audiodata or send via BT
extern __attribute__((weak)) void audio_process_i2s(uint32_t* sample, bool* continueI2S);
// record audiodata or send via BT
...
class CAudio : private AudioBuffer
{
    AudioBuffer InBuff; // instance of input buffer

public:
    CAudio(bool internalDAC = false, uint8_t channelEnabled = 3, uint8_t i2sPort = I2S_NUM_0); // #99
    ~CAudio();
    void setBufsize(int rambuf_sz, int psrambuf_sz);
    bool connecttohost(const char* host, const char* user = "", const char* pwd = "");
    bool connecttospeech(const char* host, const int port, const char* path);
...
#define ESP_ARDUINO_VERSION_PATCH 0
    ...
    enum : int8_t
    {
        AUDIOLOG_PATH_IS_NULL = -1, AUDIOLOG_FILE_NOT_FOUND = -2, AUDIOLOG_OUT_OF_MEMORY = -3,
        AUDIOLOG_FILE_READ_ERR = -4, AUDIOLOG_ERR_UNKNOWN = -127
    };
