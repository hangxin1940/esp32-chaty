...
//---------------------------------------------------------------------------------------------------------------------
CAudio::CAudio(bool internalDAC /* = false */, uint8_t channelEnabled /* = I2S_DAC_CHANNEL_BOTH_EN */, uint8_t i2sPort)
{
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::setBufsize(int rambuf_sz, int psrambuf_sz)
{
    if (InBuff.isInitialized())
    {
        log_e("CAudio::setBufsize must not be called after audio is initialized");
        return;
    }
    InBuff.setBufsize(rambuf_sz, psrambuf_sz);
};

void CAudio::initInBuff()
{
    if (!InBuff.isInitialized())
    {
...
//---------------------------------------------------------------------------------------------------------------------
esp_err_t CAudio::I2Sstart(uint8_t i2s_num)
{
...
esp_err_t CAudio::I2Sstop(uint8_t i2s_num)
{
    return i2s_stop((i2s_port_t)i2s_num);
}

//---------------------------------------------------------------------------------------------------------------------
esp_err_t CAudio::i2s_mclk_pin_select(const uint8_t pin)
{
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::setConnectionTimeout(uint16_t timeout_ms, uint16_t timeout_ms_ssl)
{
    if (timeout_ms)
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::connecttohost(const char* host, const char* user, const char* pwd)
{
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::httpPrint(const char* host)
{
    // user and pwd for authentification only, can be empty
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::setFileLoop(bool input)
{
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::UTF8toASCII(char* str)
{
...
void CAudio::printProcessLog(int r, const char* s)
{
    const char* e;
    const char* f = "";
    ...
    switch (r)
    {
    case AUDIOLOG_PATH_IS_NULL: e = "The path ore file name is empty";
        logLevel = 1;
        break;
    case AUDIOLOG_OUT_OF_MEMORY: e = "Out of memory";
        logLevel = 1;
        break;
    case AUDIOLOG_FILE_NOT_FOUND: e = "File doesn't exist: ";
        logLevel = 1;
        f = s;
        break;
    case AUDIOLOG_FILE_READ_ERR: e = "Failed to open file for reading";
        logLevel = 1;
        break;

    default: e = "UNKNOWN EVENT";
        logLevel = 3;
        break;
    }
    if (audio_log)
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::connecttoFS(fs::FS& fs, const char* path, int32_t fileStartPos)
{
    if (!path)
    {
        // guard
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::connecttospeech(const char* host, const int port, const char* path)
{
    xSemaphoreTakeRecursive(mutex_audio, portMAX_DELAY);
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::urlencode(char* buff, uint16_t buffLen, bool spacesOnly)
{
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::showID3Tag(const char* tag, const char* value)
{
    m_chbuf[0] = 0;
    // V2.2
    if (!strcmp(tag, "CNT"))
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::unicode2utf8(char* buff, uint32_t len)
{
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::latinToUTF8(char* buff, size_t bufflen)
{
...
//---------------------------------------------------------------------------------------------------------------------
size_t CAudio::readAudioHeader(uint32_t bytes)
{
...
//---------------------------------------------------------------------------------------------------------------------
int CAudio::read_WAV_Header(uint8_t* data, size_t len)
{
...
//---------------------------------------------------------------------------------------------------------------------
int CAudio::read_FLAC_Header(uint8_t* data, size_t len)
{
    static size_t headerSize;
...
//---------------------------------------------------------------------------------------------------------------------
int CAudio::read_ID3_Header(uint8_t* data, size_t len)
{
...
//---------------------------------------------------------------------------------------------------------------------
int CAudio::read_M4A_Header(uint8_t* data, size_t len)
{
    /*
           ftyp
...
//---------------------------------------------------------------------------------------------------------------------
size_t CAudio::process_m3u8_ID3_Header(uint8_t* packet)
{
    uint8_t ID3version;
    size_t id3Size;
...
// }
//----------------------------------------------------------------------------------------------------------------------
// const char *CAudio::parsePlaylist_M3U()
// {
...
// }
//----------------------------------------------------------------------------------------------------------------------
// const char *CAudio::parsePlaylist_PLS()
// {
//     uint8_t lines = m_playlistContent.size();
//     int pos = 0;
//     char *host = nullptr;
...
// }
// //----------------------------------------------------------------------------------------------------------------------
// const char *CAudio::parsePlaylist_ASX()
...
// }
// //----------------------------------------------------------------------------------------------------------------------
// const char *CAudio::parsePlaylist_M3U8()
// {
//     uint8_t lines = m_playlistContent.size();
//     bool f_begin = false;
//     uint8_t occurence = 0;
...
// }
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::STfromEXTINF(char* str)
{
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::processWebStreamTS()
{
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::processWebStreamHLS()
{
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::parseHttpResponseHeader()
{
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::parseContentType(char* ct)
{
    enum : int
    {
        CT_NONE,
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::showstreamtitle(const char* ml)
{
    // example for ml:
...
//---------------------------------------------------------------------------------------------------------------------
int CAudio::findNextSync(uint8_t* data, size_t len)
{
...
//---------------------------------------------------------------------------------------------------------------------
int CAudio::sendBytes(uint8_t* data, size_t len)
{
    int bytesLeft;
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::compute_audioCurrentTime(int bd)
{
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::printDecodeError(int r)
{
    const char* e;
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::setPinout(uint8_t BCLK, uint8_t LRC, uint8_t DOUT, int8_t DIN, int8_t MCK)
{
...
//---------------------------------------------------------------------------------------------------------------------
uint32_t CAudio::getAudioDataStartPos()
{
    if (!audiofile)
        return 0;
...
//---------------------------------------------------------------------------------------------------------------------
uint32_t CAudio::getAudioFileDuration()
{
    if (getDatamode() == AUDIO_LOCALFILE)
    {
        if (!audiofile)
...
//---------------------------------------------------------------------------------------------------------------------
uint32_t CAudio::getAudioCurrentTime()
{
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::setAudioPlayPosition(uint16_t sec)
{
    if (m_codec == CODEC_OPUS)
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::setVolumeSteps(uint8_t steps)
{
    m_vol_steps = steps;
    if (steps < 1)
...
//---------------------------------------------------------------------------------------------------------------------
uint8_t CAudio::maxVolume()
{
    return m_vol_steps;
};
//---------------------------------------------------------------------------------------------------------------------
uint32_t CAudio::getTotalPlayingTime()
{
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::setTimeOffset(int sec)
{
    if (m_codec == CODEC_OPUS)
        return false; // not impl. yet
    if (m_codec == CODEC_VORBIS)
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::setFilePos(uint32_t pos)
{
    if (m_codec == CODEC_OPUS)
        return false; // not impl. yet
    if (m_codec == CODEC_VORBIS)
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::audioFileSeek(const float speed)
{
    // 0.5 is half speed
    // 1.0 is normal speed
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::setSampleRate(uint32_t sampRate)
{
    if (!sampRate)
...
uint32_t CAudio::getSampleRate()
{
    return m_sampleRate;
}

//---------------------------------------------------------------------------------------------------------------------
bool CAudio::setBitsPerSample(int bits)
{
    if ((bits != 16) && (bits != 8))
        return false;
...
uint8_t CAudio::getBitsPerSample()
{
    return m_bitsPerSample;
}

//---------------------------------------------------------------------------------------------------------------------
bool CAudio::setChannels(int ch)
{
    if ((ch < 1) || (ch > 2))
        return false;
    m_channels = ch;
...
uint32_t CAudio::getBitRate(bool avg)
{
    if (avg)
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::setI2SCommFMT_LSB(bool commFMT)
{
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::computeVUlevel(int16_t sample[2])
{
    ...
    {
        // lambda, inner function, compute the average of 8 samples
        uint16_t av = 0;
        for (int i = 0; i < 8; i++)
        {
            av += sampArr[i];
    ...
    {
        // lambda, inner function, compute the largest of 8 samples
        uint16_t maxValue = 0;
        for (int i = 0; i < 8; i++)
        {
...
//---------------------------------------------------------------------------------------------------------------------
bool CAudio::playSample(int16_t sample[2])
{
    if (getBitsPerSample() == 8)
    {
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::setTone(int8_t gainLowPass, int8_t gainBandPass, int8_t gainHighPass)
{
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::setBalance(int8_t bal)
{
    // bal -16...16
    if (bal < -16)
        bal = -16;
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::setVolume(uint8_t vol, uint8_t curve)
{
...
//---------------------------------------------------------------------------------------------------------------------
uint8_t CAudio::getVolume()
{
    return m_vol;
}

//---------------------------------------------------------------------------------------------------------------------
uint8_t CAudio::getI2sPort()
{
    return m_i2s_num;
}

//---------------------------------------------------------------------------------------------------------------------
void CAudio::computeLimit()
{
    ...
    case 1: // logarithmic
        ...
        if (m_vol > 0)
        {
            v = m_vol * ((std::exp(log1 + (m_vol - 1) * (std::log(m_vol_steps) - log1) / (m_vol_steps - 1))) /
                m_vol_steps) / m_vol_steps;
        }
        else
        {
            v = 0;
...
int32_t CAudio::Gain(int16_t s[2])
{
    int32_t v[2];
...
//---------------------------------------------------------------------------------------------------------------------
uint32_t CAudio::inBufferFilled()
{
...
//---------------------------------------------------------------------------------------------------------------------
void CAudio::IIR_calculateCoefficients(int8_t G0, int8_t G1, int8_t G2)
{
...
//---------------------------------------------------------------------------------------------------------------------
int16_t* CAudio::IIR_filterChain0(int16_t iir_in[2], bool clear)
{
    // Infinite Impulse Response (IIR) filters
    ...
    outSample[LEFTCHANNEL] = m_filter[0].a0 * inSample[LEFTCHANNEL] + m_filter[0].a1 * m_filterBuff[0][z1][in][
            LEFTCHANNEL] + m_filter[0].a2 * m_filterBuff[0][z2][in][LEFTCHANNEL] - m_filter[0].b1 * m_filterBuff[0][z1][
            out]
        [LEFTCHANNEL] - m_filter[0].b2 * m_filterBuff[0][z2][out][LEFTCHANNEL];
    ...
    outSample[RIGHTCHANNEL] = m_filter[0].a0 * inSample[RIGHTCHANNEL] + m_filter[0].a1 * m_filterBuff[0][z1][in][
        RIGHTCHANNEL] + m_filter[0].a2 * m_filterBuff[0][z2][in][RIGHTCHANNEL] - m_filter[0].b1 * m_filterBuff[0][z1][
        out][RIGHTCHANNEL] - m_filter[0].b2 * m_filterBuff[0][z2][out][RIGHTCHANNEL];
...
//---------------------------------------------------------------------------------------------------------------------
int16_t* CAudio::IIR_filterChain1(int16_t iir_in[2], bool clear)
{
    // Infinite Impulse Response (IIR) filters
    ...
    outSample[LEFTCHANNEL] = m_filter[1].a0 * inSample[LEFTCHANNEL] + m_filter[1].a1 * m_filterBuff[1][z1][in][
            LEFTCHANNEL] + m_filter[1].a2 * m_filterBuff[1][z2][in][LEFTCHANNEL] - m_filter[1].b1 * m_filterBuff[1][z1][
            out]
        [LEFTCHANNEL] - m_filter[1].b2 * m_filterBuff[1][z2][out][LEFTCHANNEL];
    ...
    outSample[RIGHTCHANNEL] = m_filter[1].a0 * inSample[RIGHTCHANNEL] + m_filter[1].a1 * m_filterBuff[1][z1][in][
        RIGHTCHANNEL] + m_filter[1].a2 * m_filterBuff[1][z2][in][RIGHTCHANNEL] - m_filter[1].b1 * m_filterBuff[1][z1][
        out][RIGHTCHANNEL] - m_filter[1].b2 * m_filterBuff[1][z2][out][RIGHTCHANNEL];
...
//---------------------------------------------------------------------------------------------------------------------
int16_t* CAudio::IIR_filterChain2(int16_t iir_in[2], bool clear)
{
    // Infinite Impulse Response (IIR) filters
    ...
    outSample[LEFTCHANNEL] = m_filter[2].a0 * inSample[LEFTCHANNEL] + m_filter[2].a1 * m_filterBuff[2][z1][in][
            LEFTCHANNEL] + m_filter[2].a2 * m_filterBuff[2][z2][in][LEFTCHANNEL] - m_filter[2].b1 * m_filterBuff[2][z1][
            out]
        [LEFTCHANNEL] - m_filter[2].b2 * m_filterBuff[2][z2][out][LEFTCHANNEL];
    ...
    outSample[RIGHTCHANNEL] = m_filter[2].a0 * inSample[RIGHTCHANNEL] + m_filter[2].a1 * m_filterBuff[2][z1][in][
        RIGHTCHANNEL] + m_filter[2].a2 * m_filterBuff[2][z2][in][RIGHTCHANNEL] - m_filter[2].b1 * m_filterBuff[2][z1][
        out][RIGHTCHANNEL] - m_filter[2].b2 * m_filterBuff[2][z2][out][RIGHTCHANNEL];
...
//----------------------------------------------------------------------------------------------------------------------
bool CAudio::ts_parsePacket(uint8_t* packet, uint8_t* packetStart, uint8_t* packetLength)
{
...
//----------------------------------------------------------------------------------------------------------------------
uint16_t CAudio::readMetadata(uint16_t maxBytes, bool first)
{
...
//----------------------------------------------------------------------------------------------------------------------
size_t CAudio::chunkedDataTransfer(uint8_t* bytes)
{
...
//----------------------------------------------------------------------------------------------------------------------
boolean CAudio::streamDetection(uint32_t bytesAvail)
{
...
void CAudio::seek_m4a_ilst()
{
    ...
    auto atomItems = [&](uint32_t startPos)
    {
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t CAudio::m4a_correctResumeFilePos(uint32_t resumeFilePos)
{
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t CAudio::flac_correctResumeFilePos(uint32_t resumeFilePos)
{
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t CAudio::mp3_correctResumeFilePos(uint32_t resumeFilePos)
{
...
//----------------------------------------------------------------------------------------------------------------------
uint8_t CAudio::determineOggCodec(uint8_t* data, uint16_t len)
{
